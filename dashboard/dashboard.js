...
function loadSettings() {
    try {
        ...
        if (saved) {
            ...
            document.getElementById('input-refresh-interval').value = config.refreshInterval;
        }
    } catch (e) {
...
function saveSettings() {
    ...
    config.refreshInterval = parseInt(document.getElementById('input-refresh-interval').value) || 30;
...
async function refreshAll() {
    ...
    try {
        ...
        setStatus(`✓ Connected — ${devices.length} devices, ${events.length} events`, 'connected');
    } catch (err) {
...
function renderBattery(level) {
    ...
    const cls = level > 50 ? 'battery-high' : level > 20 ? 'battery-mid' : 'battery-low';
...
function renderEvents() {
    ...
    log.innerHTML = allEvents.slice(0, 200).map(e => {
...
function filterEvents() {
    ...
    items.forEach(item => {
        ...
        const matchSearch = !search || item.dataset.text.toLowerCase().includes(search);
...
function summarizeJsonData(obj) {
    const parts = [];
    if (obj.totalPacketsSent !== undefined) parts.push(`pkts↑${obj.totalPacketsSent}`);
    if (obj.totalPacketsReceived !== undefined) parts.push(`pkts↓${obj.totalPacketsReceived}`);
    if (obj.currentNeighborCount !== undefined) parts.push(`peers:${obj.currentNeighborCount}`);
    ...
    if (parts.length === 0) {
        ...
        return keys.map(k => `${k}:${String(obj[k]).substring(0, 30)}`).join(', ');
    }
    return parts.join(' | ');
}
...
function renderRelayQueue() {
    ...
    document.getElementById('relay-messages').textContent = allRelayMessages.length;

    if (allRelayMessages.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="empty-row">No relay messages in queue</td></tr>';
        return;
    }

    tbody.innerHTML = allRelayMessages.map(m => {
...
function renderTestResults() {
    ...
    const testEvents = allEvents.filter(e => (e.eventType || e.type || '').includes('TEST'));

    if (testEvents.length === 0) {
        container.innerHTML = '<div class="empty-row">No test results available</div>';
        return;
    }
...
function updateSummaryCards() {
    ...
    document.getElementById('active-devices').textContent =
        allDevices.filter(d => (now - (d.lastSeen || d.timestamp || 0)) < fifteenMin).length;
    document.getElementById('total-events').textContent = allEvents.length;

    // Sum total connections across all devices
    const totalConns = allDevices.reduce((sum, d) =>
        sum + (d.totalConnectionsEstablished || 0), 0);
...
function initCharts() {
    ...
    charts.packets = new Chart(document.getElementById('chart-packets'), {
        ...
        data: {
            labels: [],
            datasets: [
                { label: 'Sent', data: [], borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.1)', fill: true, tension: 0.3 },
                { label: 'Received', data: [], borderColor: '#22c55e', backgroundColor: 'rgba(34,197,94,0.1)', fill: true, tension: 0.3 },
                { label: 'Forwarded', data: [], borderColor: '#a855f7', backgroundColor: 'rgba(168,85,247,0.1)', fill: true, tension: 0.3 },
            ]
        },
        options: {
            responsive: true,
...
function updatePacketChart() {
    ...
    const labels = stats.map(s => new Date(s.timestamp || 0).toLocaleTimeString());
    const sent = [];
    const recv = [];
    const fwd = [];
...
function drawTopology() {
    ...
    ctx.lineWidth = 1.5;
    const nameToIndex = {};
    nodes.forEach((n, i) => { nameToIndex[n.name] = i; });

    nodes.forEach((node, i) => {
        if (Array.isArray(node.neighbors)) {
            node.neighbors.forEach(neighbor => {
                const nName = typeof neighbor === 'string' ? neighbor.substring(0, 12) : '';
    ...
    nodes.forEach(node => {
        ...
        ctx.fillStyle = node.isActive ? 'rgba(34, 197, 94, 0.3)' : 'rgba(100, 116, 139, 0.3)';
        ctx.fill();
